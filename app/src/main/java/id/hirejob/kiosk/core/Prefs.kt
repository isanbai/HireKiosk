...
object Prefs {
    ...
    fun pin(ctx: Context): String = ctx.getSharedPreferences(FILE, 0).getString(K_PIN, DEFAULT_PIN)!!
    ...
    suspend fun setUsbHidKey(
        ctx: Context,
        v: String,
    ) = ctx.dataStore.edit { it[USB_HID] = v }

    fun videoUri(ctx: Context) =
        ctx.dataStore.data.map {
            it[VIDEO_URI] ?: "android.resource://id.hirejob.kiosk/${R.raw.anim}"
        }

    suspend fun setVideoUri(
        ctx: Context,
        v: String,
    ) = ctx.dataStore.edit { it[VIDEO_URI] = v }

    fun imageUri(ctx: Context) =
        ctx.dataStore.data.map {
            it[IMAGE_URI] ?: "android.resource://id.hirejob.kiosk/${R.drawable.idle}"
        }

    suspend fun setImageUri(
        ctx: Context,
        v: String,
    ) = ctx.dataStore.edit { it[IMAGE_URI] = v }

    fun triggerSource(ctx: Context) = ctx.dataStore.data.map { it[TRIGGER_SOURCE] ?: TriggerType.VOLUME.name }

    suspend fun setTriggerSource(
        ctx: Context,
        v: String,
    ) = ctx.dataStore.edit { it[TRIGGER_SOURCE] = v }

    fun loopVideo(ctx: Context) = ctx.dataStore.data.map { it[LOOP_VIDEO] ?: true }

    suspend fun setLoopVideo(
        ctx: Context,
        v: Boolean,
    ) = ctx.dataStore.edit { it[LOOP_VIDEO] = v }
